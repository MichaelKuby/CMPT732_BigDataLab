...
def parse_line(record):
    ...
    if match:
        ...
        # throw away this record
        return


def main(inputs):
    logs = sc.textFile(inputs)
    parsed_logs = logs.map(parse_line).filter(lambda x: x is not None)
    logs_df = spark.createDataFrame(parsed_logs, logs_schema)
    groups = logs_df.groupBy(logs_df['host_name']).agg(F.count(logs_df['req_path']), F.sum(logs_df['bytes_transferred']))
